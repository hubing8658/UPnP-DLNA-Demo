...
/**
 * 日志记录工具类 可以通过修改是否启用调试模式、日志级别、是否记录日志到文件, 灵活的记录日志。 记录日志到文件需要配置权限：android.permission.WRITE_EXTERNAL_STORAGE
 * 
 * @author kjxu
...
public class LogUtil {
     ...
     * 日志文件名称绝对路径
     */
    private static String LOGFILENAME = Environment.getExternalStorageDirectory().getPath() + "/smarthome/log/app_name.log";
    ...
    static {
        ...
        try {
            ...
            is = LogUtil.class.getResourceAsStream("/assets/ua.properties");
            p.load(is);

            Log.d(TAG, "load config: " + p.toString());

            ADB = Boolean.parseBoolean(p.getProperty("ua.log.ADB", "true"));
            LOG_DEGREE = Integer.parseInt(p.getProperty("ua.log.LOG_DEGREE", "2"));
            String logName = p.getProperty("ua.log.LOGFILENAME");
    ...
    public static void w(String tag, String msg, Throwable tr) {
    	if (ADB && LOG_DEGREE <= Log.WARN) {
            ...
            writeLogToFile(Log.WARN, tag, msgStr, tr);
        }
    }
    ...
    private synchronized static void initWriter() {
        ...
        try {
            ...
            if (mExternalStorageWriteable) {
            	...
            	// log目录不存在时，创建log层级目录
                File logDir = new File(LOGFILENAME.substring(0, LOGFILENAME.lastIndexOf("/")));
                if (!logDir.exists()) {
                	logDir.mkdirs();
                }
